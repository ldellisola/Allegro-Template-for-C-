...
namespace Allw {
	...
	AllegroWindow::AllegroWindow(float w, float h, ALLEGRO_EVENT_QUEUE* evQueue, bool open, std::string name, std::string icon)
	{
		...
		this->layout = new UI::AllegroLayout(w, h, Color::AllegroColor("black"), UI::LayoutDrawMode::Mid);
	...
	void AllegroWindow::insertLayout(UI::AllegroLayout& layout)
	{
	...
	void AllegroWindow::deleteLayout()
	{
		...
		this->layout = new UI::AllegroLayout(this->width, this->height, Color::AllegroColor("black"));
	}

	void AllegroWindow::setLayoutDrawMode(UI::LayoutDrawMode mode)
	{
		if (layout != nullptr)
			layout->setDrawMode(mode);
	}

	void AllegroWindow::addBox(UI::AllegroBox & box)
	{
		if (layout != nullptr)
			layout->addBox(&box);
	}

	void AllegroWindow::removeBox(UI::AllegroBox & box)
	{
		if (layout != nullptr)
			layout->removeBox(box);
	}

	void AllegroWindow::loadBackground(Color::AllegroColor color)
	{
		if (layout != nullptr)
	...
	void AllegroWindow::loadImageBackground(UI::AllegroBox & image)
	{
		if (layout != nullptr)
			layout->loadImageBackground(image);
	}
	...
	void AllegroWindow::insertMenuItem(std::string title, uint16_t uniqueID, UI::MenuFlags flag, unsigned int pos)
	{
		if (menu != nullptr) {
	...
	void AllegroWindow::insertMenuSubItemp(uint16_t parentID, std::string title, uint16_t uniqueID, UI::MenuFlags flag, unsigned int pos)
	{
		if (menu != nullptr)
			this->menu->insertSubItem(parentID, title, uniqueID, flag, pos);
	}

	void AllegroWindow::appendMenuItem(std::string title, uint16_t uniqueID, UI::MenuFlags flag)
	{
	...
	void AllegroWindow::appendMenuSubItem(uint16_t parentID, std::string title, uint16_t uniqueID, UI::MenuFlags flag)
	{
		if (menu != nullptr)
			menu->appendSubItem(parentID, title, uniqueID, flag);
	}

	void AllegroWindow::setMenuItemFlag(uint16_t uniqueID, UI::MenuFlags flag)
	{
		if (menu != nullptr)
	...
	UI::MenuFlags AllegroWindow::getMenuItemFlag(uint16_t uniqueID)
	{
		if (menu != nullptr)
			return menu->getItemFlag(uniqueID);
		else
			return UI::MenuFlags::Error;
	}
