...
namespace Allw {
	...
	*/
	...
	AllegroWindow(float w, float h, ALLEGRO_EVENT_QUEUE  *evQueue = nullptr, bool open = true, std::string name = "", std::string icon = "");
		...
		void insertLayout(UI::AllegroLayout& layout);

		void deleteLayout();

		void setLayoutDrawMode(UI::LayoutDrawMode mode);

		// It adds an AllegroBox to the window. It will be printed everytime the window is updatesd by its own draw function
		void addBox(UI::AllegroBox& box);
		...
		void insertMenuItem(std::string title, uint16_t uniqueID, UI::MenuFlags flag, unsigned int pos);
		...
		//		- MenuFlags flag: The kind of item it will be. It can be Enabled, Disabled,Checked and checkBox
		//		- unsigned int pos: the position where the item will be placed.
		void insertMenuSubItemp(uint16_t parentID, std::string title, uint16_t uniqueID, UI::MenuFlags flag, unsigned int pos);
		...
		//							 if the user pressed this item, it will return the ID int event.user.data1
		//		- MenuFlags flag: The kind of item it will be. It can be Enabled, Disabled,Checked and checkBox
		void appendMenuItem(std::string title, uint16_t uniqueid, UI::MenuFlags flag);
		...
		//							 if the user pressed this item, it will return the ID int event.user.data1
		//		- MenuFlags flag: The kind of item it will be. It can be Enabled, Disabled,Checked and checkBox
		void appendMenuSubItem(uint16_t parentID, std::string title, uint16_t uniqueID, UI::MenuFlags flag);

		// It sets a new flag for a given item
		void setMenuItemFlag(uint16_t uniqueID, UI::MenuFlags flag);
		...
		UI::MenuFlags getMenuItemFlag(uint16_t uniqueID);
	...
	private:
		...
		// Draw
		UI::AllegroLayout * layout = nullptr;
